    ...
    public void ReadFile(File inputFile)
        ...
        while (mOffset + 8 <= mFileSize) {
            ...
            if (chunkHeader[0] == 'f' &&
                ...
                FrameGains = new FrameAudioData[mNumFrames];
                ...
                while (i < chunkLen) {
                    ...
                    short[] arr = new short[oneFrameBytes/2];
                    for (int j = 0; j < oneFrameBytes; j += 2)
                        arr[j/2] = get2Data(oneFrame, j, j+2);

                    data = new FrameAudioData(arr.length/mChannels, mChannels == 1);
                    data.sampleRate = mSampleRate;
                    if (!data.isMono)
                        for (int j = 0 ; j < arr.length; j+= mChannels){
                            data.LeftData[j/mChannels] = arr[j];
                            data.RiteData[j/mChannels] = arr[j+1];
                        }
                    else{
                        data.RiteData = null;
                        data.LeftData = arr;
